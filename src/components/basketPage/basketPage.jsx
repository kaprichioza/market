...
export const BasketPage = props => {
  ...
  const sum = selectedItems.reduce(
    (accum, currentItem) => accum + currentItem.price,
    0
  )
  const quantityItems = basketData.length
  const descriptionText = `${quantityItems} ${pluralize(quantityItems, [
    'товар',
    'товара',
    'товаров'
  ])}`
      ...
      {isLoading ? (
        ...
        <>
          ...
          <OrderDetails
            ...
            onCartItemToggle={onCartItemToggle}
            cartItems={basketData}
          />
          <Summary
            onOrderFromCart={props.onOrderFromCart}
